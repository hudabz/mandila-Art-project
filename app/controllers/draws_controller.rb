class DrawsController < ApplicationController
  ...
  # POST /draws.json
  def create
    @draw = current_artist.draws.new(draw_params)
    # @draw = current_artist.image.attach(draw_params)
    # @draw = current_artist.image.attach(params[:image])
  ...
  private
    ...
    def set_draw
      @draw = current_artist.draws.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def draw_params 
      params.require(:draw).permit(:tools, :description, :painting, :image) 
    end
end
